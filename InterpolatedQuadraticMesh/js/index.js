...
function mainLoop() {
  ...
  let diff = (t - last_t)/INTER;
  for (let i = 1; i <= INTER; ++i) {
    let line_t = i*diff + last_t;
    penDown(canvas.width + Math.sin(line_t) * SIZE,            canvas.height + Math.cos(line_t) * SIZE, line_t);
  }
  last_t = t;
...
function fadeOut(color) {
  // c.clearRect(0, 0, canvas.width, canvas.height);
  c.fillStyle = generateColor(color, 0, 0, 5/INTER);
